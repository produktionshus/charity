...
let ctBlocks: Array<{ src?: string; heading?: string; lines?: string[] }> = [];
...
carUploadEl.addEventListener('change', async () => {
  ...
  for (const file of files) {
    ...
    try {
      const res = await fetch('/api/upload', { method: 'POST', body: fd });
      const data = await res.json();
      if (data?.filename) carImages.push({ src: `/assets/media/${data.filename}` });
...
ctUploadEl.addEventListener('change', async () => {
  ...
  for (const file of files) {
    ...
    try {
      const res = await fetch('/api/upload', { method: 'POST', body: fd });
      const data = await res.json();
      if (data?.filename) ctBlocks.push({ src: `/assets/media/${data.filename}`, lines: [] });
    } catch (e: any) { statusEl.textContent = 'Upload failed: ' + e.message; }
  }
  ctUploadEl.value = '';
  renderContestBlocks();
